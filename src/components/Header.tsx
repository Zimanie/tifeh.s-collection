...
export default function Header() {
  ...
  return <>
    <div className="announcement">Complimentary delivery on orders over ₦150,000 · Easy exchanges within 7 days</div>
    <header className="site-header"><div className="header-inner"><button className="mobile-menu-button" type="button" aria-label="Open menu" onClick={() => setMenuOpen(true)}><Menu size={21} /></button><nav className="desktop-nav">{links.map(([label, to]) => <NavLink key={label} to={to}>{label}</NavLink>)}</nav><Link to="/" className="brand">Tifeh.s <span>Collection</span></Link><div className="header-actions"><button type="button" aria-label="Search" onClick={() => setSearchOpen(!searchOpen)}><Search size={18} /></button><Link to="/wishlist" aria-label="Wishlist" className="optional-action"><Heart size={18} />{wishlist.length > 0 && <b>{wishlist.length}</b>}</Link><Link to="/cart" aria-label="Shopping bag" className="bag-action"><ShoppingBag size={19} />{cartItemCount > 0 && <b>{cartItemCount}</b>}</Link><select value={currency} aria-label="Currency" onChange={(event) => setCurrency(event.target.value as keyof typeof CURRENCIES)}>{Object.keys(CURRENCIES).map((code) => <option key={code}>{code}</option>)}</select></div></div>{searchOpen && <div className="search-row"><Search size={16} /><input autoFocus placeholder="Search the collection" /></div>}</header>
